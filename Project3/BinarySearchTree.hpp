...
private:
    ...
    bool find(const Comparable &c, BinaryNode* n, uint32_t *pdepth) const {
        if (n == nullptr) {
        ...
        if (c < n->value) {
            ...
            ++*pdepth;
            return find(c, n->leftChild, pdepth);
        }
        if (n->value < c) {
            // Value is greater than current node. Go to node's right child.
            ++*pdepth;
            return find(c, n->rightChild, pdepth);
        }
...
public:
    ...
    bool find(const Comparable &c, uint32_t *pdepth) const {
        // calls private helper function
        return find(c, root, pdepth);
    }
